...
source_dir = "../source/"
output_dir = "../../"
SOURCE_FOLDERS = ["deaths_number_rate", "incidence_number_rate"]
CONF = {
    "split_by": "sex",
...
class QueryProcessor:
    ...
    def serve_datapoint(self, df, concept):
        ...
        for g_ in product(causes, sexes):
            ...
            file_name = (
                "ddf--datapoints--" + concept + "--by--" + "--".join(by) + ".csv"
            )
            file_path = osp.join(output_dir, file_name)
            df_[concept] = df_[concept].map(formatter)
            df_[cols_new].sort_values(by=["location", "sex", "age", "year"]).to_csv(
    ...
    def get_measures(self):
        ...
        # Load a sample file to determine available measures/metrics
        source_path = os.path.join("../source/", self.query["id"])
        sample_file = next(f for f in os.listdir(source_path) if f.endswith(".zip"))
        sample_data = self.load_data(sample_file)

        available_measures = sample_data["measure"].unique()
        available_metrics = sample_data["metric"].unique()

        measure_metric_combinations = product(available_measures, available_metrics)
    ...
    def run(self):
        ...
        data_full = dd.from_delayed(
            [
                dask.delayed(self.load_data)(f)
        ...
        all_measures = list()
        # Determine available measures/metrics from the data
        sample_file = next(f for f in os.listdir(source_dir) if f.endswith(".zip"))
        sample_data = self.load_data(sample_file)
        available_measures = sample_data["measure"].unique()
        available_metrics = sample_data["metric"].unique()
        measure_metric_combinations = product(available_measures, available_metrics)
        for g in measure_metric_combinations:
            ...
            df = data_full.loc[
                (data_full.measure == g[0]) & (data_full.metric == g[1]), cols
            ].compute()
            cols_rename = by.copy()
...
def serve_entities(md):
    ...
    locations = locations[~pd.isnull(locations["id"])].drop(columns=["location_id"])
...
def main():
    ...
    for folder_id in SOURCE_FOLDERS:
        # Create a minimal query object with just the ID
        query = {"id": folder_id}
        p = QueryProcessor(query, md, config=CONF)
    ...
    dis_cdf.sort_values(by="concept").to_csv(
        "../../ddf--concepts--discrete.csv", index=False
    )
